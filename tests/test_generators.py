...
def test_filter_by_currency_is_generator():
...
def test_transaction_descriptions_skips_empty():
...
def test_transaction_descriptions_is_generator():
...
def test_generator_count(simple_range):
    assert len(simple_range) == 5

"""Проверяем правильность формата XXXX XXXX XXXX XXXX"""
def test_format_of_card_number(simple_range):
...
def test_zero_start_min_number_card():
